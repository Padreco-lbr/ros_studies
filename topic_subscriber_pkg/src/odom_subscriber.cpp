...
void counterCallback(const nav_msgs::Odometry::ConstPtr &msg) {
  ROS_INFO("Id: %s", msg->child_frame_id.c_str());
  ROS_INFO("Header Id: %s", msg->header.frame_id.c_str());
  ...
  ROS_INFO("Twist angular: [%.3f, %.3f, %.3f]", msg->twist.twist.angular.x,
           msg->twist.twist.angular.y, msg->twist.twist.angular.z);
  ROS_INFO("Twist linear: [%.3f, %.3f, %.3f]", msg->twist.twist.linear.x,
